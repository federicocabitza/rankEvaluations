...
def usage(): 
	help = ''
	help = """ 
	This is the help docstring of RankEvaluation. This is a very simple (and sometimes even candid) Python script that performs a ranking of evaluations expressed in an ordinal scale.

	Requirements: Python 3.12.x, scipy, ranking and numpy. 
	Input options: 
...
def main():
	...
	if nomefile =='':
		nomefile= sys.argv[1]
	with open(nomefile, 'rt', encoding='utf-8') as csvfile:
	...
	for variabile in podio:
		...
		singololog = str(k+1) + ') Analysis for ' + str(items[k]) + '\n\t' + 'No. of cases: ' + str(numerovalutazioni[k]) + '\n\t' + 'Mean position: ' + str(medie[k]) + '\n\t' + 'Distribution in the first 3 ranks: ' + str(variabile) + '\n\t' + 'Distribution in the 2 priority levels: ' + str(fasce[k]) + '\n\t' + "The analysis of uniformity bwt the first 3 ranks is: Chi Square: %.4f, P-value: %.4f " % (chipodio, pvaluepodio) + '\n\t' + "The analysis of uniformity bwt priority levels is: Chi Square: %.4f, P-value: %.4f " % (chifasce, pvaluefasce) + '\n'
		if pvaluefasce < .05:
			singololog = singololog + "\t" + "The level assignment CAN be considered statistically significant.\n"
		else:
			singololog = singololog + "\t" +"The level assignment CAN NOT be considered statistically significant.\n"
		if debugmode:
				print(singololog)
		logfile = logfile + singololog
		summary = summary + singololog
